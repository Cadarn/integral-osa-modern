...
def get_cost_explorer_breakdown():
    ...
    for group in usage_resp.get("ResultsByTime", [{}])[0].get("Groups", []):
        ...
        if cost > 0.0001:
            usage_breakdown.append({"usage_type": utype, "cost": cost, "quantity": qty, "unit": unit})
...
def main():
    console.print(Panel("[bold green]Actual AWS Incurred Costs & Billing Analysis[/bold green]\n"
