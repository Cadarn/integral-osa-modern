...
def scw(
    ...
    if len(scw_id) != 12:
        console.print(f"[red]Error: ScW ID must be 12 digits (e.g. 197200240010), got {scw_id}[/red]")
    ...
    console.print(f"[bold blue]Fetching Science Window {scw_id} (Rev {rev}) into {scw_dir}...[/bold blue]")
    ...
    console.print(f"[bold green]Science Window {scw_id} ready ({downloaded_count}/{len(key_files)} files).[/bold green]")
...
def info():
    ...
    table.add_row("ISDC_REF_CAT", os.environ.get("ISDC_REF_CAT", "Default (/data/cat/hec/...)"), "OK")
