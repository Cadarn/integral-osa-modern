...
@app.command()
def report(
    arm_100_seconds: float = typer.Option(
        2350.0, "--arm-seconds", help="Observed ARM64 Graviton3 100-ScW duration (sec)"
    ),
    x86_100_seconds: float = typer.Option(
        2800.0, "--x86-seconds", help="Observed x86_64 Sapphire Rapids 100-ScW duration (sec)"
    ),
    ...
    console.print(
        Panel(
            "[bold green]AWS Cloud Compute & Data Architecture Cost Accounting[/bold green]\nRegion: [cyan]us-east-1 (N. Virginia)[/cyan]",
            title="Cost Model",
        )
    )
    ...
    t_stage = Table(
        title="1. One-Time Data Ingestion & Storage Costs (Rev 0060, ~2.5 GB, 2,000 files)"
    )
    ...
    t_stage.add_row(
        "Ephemeral Stager Compute (t4g.medium)",
        "$0.0101 / hr (Spot)",
        f"{stager_seconds:.0f} sec ({stager_hours:.3f} hr)",
        f"${stager_compute:.5f}",
    )
    t_stage.add_row(
        "Stager Root EBS Disk (40 GB gp3)",
        "$0.08 / GB-mo (pro-rated)",
        f"{stager_seconds:.0f} sec",
        f"${stager_ebs:.5f}",
    )
    t_stage.add_row(
        "S3 PUT API Requests", "$0.005 per 1,000 PUTs", "2,000 requests", f"${s3_put_cost:.5f}"
    )
    t_stage.add_row(
        "S3 Standard Storage (per month)",
        "$0.023 / GB-month",
        "2.5 GB stored",
        f"${s3_storage_month:.4f} / mo",
    )

    total_one_time_stage = stager_compute + stager_ebs + s3_put_cost
    t_stage.add_row(
        "[bold]Total Ingestion Phase[/bold]",
        "",
        "",
        f"[bold green]${total_one_time_stage:.4f}[/bold green]",
    )
    ...
    t_triple.add_row(
        "ARM64 3x Repeats (Full Rev 0060)",
        "3x c7g.4xlarge in parallel",
        f"{arm_100_seconds / 60:.1f} min",
        f"${arm_cost['total_cost'] * 3:.3f}",
    )
    t_triple.add_row(
        "x86_64 3x Repeats (Full Rev 0060)",
        "3x c7i.4xlarge in parallel",
        f"{x86_100_seconds / 60:.1f} min",
        f"${x86_cost['total_cost'] * 3:.3f}",
    )
    t_triple.add_row(
        "Combined Both Architectures (6 Runs)",
        "6 Spot Instances Total",
        f"{max(arm_100_seconds, x86_100_seconds) / 60:.1f} min",
        f"${(arm_cost['total_cost'] + x86_cost['total_cost']) * 3:.3f}",
    )
    ...
    t_proj = Table(
        title="4. Mission-Scale Projection: Processing 1,000 INTEGRAL Revolutions (~100,000 ScWs)"
    )
    ...
    t_proj.add_row(
        "Archive Ingestion to S3",
        "2.5 TB (2,000,000 files)",
        "$10.00 (PUT APIs)",
        "$10.00 (PUT APIs)",
    )
    t_proj.add_row(
        "S3 Storage (per year)", "2.5 TB retained in S3", "$690.00 / year", "$690.00 / year"
    )
    t_proj.add_row(
        "Full Pipeline Reduction (Spot)",
        "1,000 Revolutions processed",
        f"${arm_cost['total_cost'] * 1000:.2f}",
        f"${x86_cost['total_cost'] * 1000:.2f}",
    )
    t_proj.add_row(
        "Cost Savings with ARM64",
        "Graviton3 vs Sapphire Rapids",
        "BASELINE",
        f"+${(x86_cost['total_cost'] - arm_cost['total_cost']) * 1000:.2f} (+{(x86_cost['total_cost'] / arm_cost['total_cost'] - 1) * 100:.1f}%)",
    )
