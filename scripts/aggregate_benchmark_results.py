...
def main():
    ...
    console.print(Panel(f"[bold cyan]INTEGRAL Cloud Benchmark Fleet Aggregator (Rev 0060, {scale} ScWs to IMA2)[/bold cyan]"))
    ...
    console.print(f"Discovered results in S3: [bold green]{len(arm_runs)} ARM64[/bold green] | [bold yellow]{len(x86_runs)} x86_64[/bold yellow]")
    ...
    for arch, r in all_entries:
        ...
        if runs_list:
            re = runs_list[0]
            status = "[green]SUCCESS[/green]" if re.get("returncode") == 0 else f"[red]FAIL ({re.get('returncode')})[/red]"
    ...
    summary_table.add_row("Nodes Completed", f"{arm_stats['success_count']}/{arm_stats['count']}", f"{x86_stats['success_count']}/{x86_stats['count']}", "—")
    summary_table.add_row("Docker Image Pull", format_stats(arm_stats["pull_times"]), format_stats(x86_stats["pull_times"]), "—")
    summary_table.add_row("S3 Data Transfer", format_stats(arm_stats["transfer_times"]), format_stats(x86_stats["transfer_times"]), "—")
    ...
    pipe_ratio = f"{arm_pipe_mean / x86_pipe_mean:.2f}x faster (x86_64)" if (arm_pipe_mean and x86_pipe_mean) else "N/A"
    ...
    pscw_ratio = f"{arm_pscw_mean / x86_pscw_mean:.2f}x faster (x86_64)" if (arm_pscw_mean and x86_pscw_mean) else "N/A"

    summary_table.add_row("Pipeline Runtime (COR->IMA2)", format_stats(arm_stats["pipeline_times"]), format_stats(x86_stats["pipeline_times"]), pipe_ratio)
    summary_table.add_row("Per-ScW Runtime", format_stats(arm_stats["per_scw_times"]), format_stats(x86_stats["per_scw_times"]), pscw_ratio)
    
    target_name = arm_stats["source_name"] or x86_stats["source_name"] or "Target"
    summary_table.add_row(f"{target_name} Detection Sig", 
                          format_stats(arm_stats['source_sigs'], unit="σ"), 
                          format_stats(x86_stats['source_sigs'], unit="σ"), 
                          f"Δ = {abs(np.mean(arm_stats['source_sigs']) - np.mean(x86_stats['source_sigs'])):.3f}σ" if arm_stats['source_sigs'] and x86_stats['source_sigs'] else "N/A")
    summary_table.add_row(f"{target_name} Net Flux", 
                          format_stats(arm_stats['source_fluxes'], unit=" c/s"), 
                          format_stats(x86_stats['source_fluxes'], unit=" c/s"), 
                          f"Agreement: {100 * (1 - abs(np.mean(arm_stats['source_fluxes']) - np.mean(x86_stats['source_fluxes'])) / np.mean(arm_stats['source_fluxes'])):.2f}%" if arm_stats['source_fluxes'] and x86_stats['source_fluxes'] else "N/A")
