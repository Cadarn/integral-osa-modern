    ...
    with fits.open(fits_ref) as hdu_ref, fits.open(fits_test) as hdu_test:
        ...
        for idx in range(len(hdu_ref)):
            ...
            if h_test.data is None:
                console.print(f"[red]HDU {idx} ({h_ref.name}) has data in ref but None in test[/red]")
                all_passed = False
                continue
            ...
            console.print(f"  HDU {idx} [{h_ref.name}]: Max Abs Diff = {max_diff:.3e}, Rel Diff = {rel_diff:.3e} -> [{color}]{status}[/{color}]")
...
def compare_catalogs(fits_ref: Path, fits_test: Path) -> bool:
    ...
    with fits.open(fits_ref) as hdu_ref, fits.open(fits_test) as hdu_test:
        ...
        console.print(f"Sources in reference: {len(sources_ref)} | Sources in test: {len(sources_test)}")
        return len(sources_ref) == len(sources_test)


@app.command()
def compare(
    ref_dir: Path = typer.Argument(..., help="Path to reference results directory (x86_64 baseline)"),
    test_dir: Path = typer.Argument(..., help="Path to test results directory (ARM64)"),
    tolerance: float = typer.Option(1e-3, "--tolerance", "-t", help="Relative floating point tolerance"),
):
    ...
    console.print(f"[bold blue]Comparing {len(ref_fits_files)} FITS products between {ref_dir} and {test_dir}...[/bold blue]")
    ...
    for ref_file in ref_fits_files:
        ...
        try:
            ...
            if is_valid:
                table.add_row(str(rel_path), "[green]VERIFIED[/green]", f"Within {tolerance:.1e} tolerance")
            else:
                table.add_row(str(rel_path), "[red]DIFF DETECTED[/red]", "Numerical difference exceeds tolerance")
                all_ok = False
    ...
    console.print(table)
    if all_ok:
        console.print("[bold green]✓ All scientific products verified identical within tolerance![/bold green]")
    else:
