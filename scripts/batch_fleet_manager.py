...
BATCHES = [
    {
        "name": "Batch 1: ARM64 10 ScWs (N=10)",
        "arch": "arm64",
        "scale": "10",
        "parallel": "10",
        "subnet": "subnet-a9e00af0",
    },
    {
        "name": "Batch 2: x86_64 10 ScWs (N=10)",
        "arch": "x86_64",
        "scale": "10",
        "parallel": "10",
        "subnet": "subnet-67212b4f",
    },
    {
        "name": "Batch 3: ARM64 25 ScWs (N=10)",
        "arch": "arm64",
        "scale": "25",
        "parallel": "10",
        "subnet": "subnet-a9e00af0",
    ...
    {
        "name": "Batch 4: x86_64 25 ScWs (N=10)",
        "arch": "x86_64",
        "scale": "25",
        "parallel": "10",
        "subnet": "subnet-67212b4f",
    ...
    {
        "name": "Batch 5: ARM64 100 ScWs Full Rev (N=3)",
        "arch": "arm64",
        "scale": "100",
        "parallel": "3",
        "subnet": "subnet-a9e00af0",
    ...
    {
        "name": "Batch 6: x86_64 100 ScWs Full Rev (N=3)",
        "arch": "x86_64",
        "scale": "100",
        "parallel": "3",
        "subnet": "subnet-67212b4f",
...
def wait_for_instances_termination(instance_ids: list[str], poll_interval: int = 20):
    console.print(
        f"[cyan]Waiting for {len(instance_ids)} instance(s) to complete and terminate: {instance_ids}[/cyan]"
    )
    while True:
        ...
        if not non_terminated:
            console.print(
                "[bold green]✓ All instances in batch have completed and terminated![/bold green]"
            )
            break

        console.print(
            f"[{datetime.now(timezone.utc).strftime('%H:%M:%S')}] Active instances remaining: {len(non_terminated)}/{len(instance_ids)} (states: {set(non_terminated)})..."
        )
...
def launch_and_manage():
    for idx, b in enumerate(BATCHES, 1):
        ...
        cmd = [
            ...
            "run",
            "python",
            "scripts/run_aws_cloud_benchmark.py",
            "--arch",
            b["arch"],
            "--scale",
            b["scale"],
        ...
        console.print(
            f"[green]Successfully tracked {len(instance_ids)} instances for {b['name']}[/green]"
        )
