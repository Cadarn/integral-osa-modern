    ...
    for obj in resp.get("Contents", []):
        ...
        for scale in ["10", "25", "100"]:
            if f"_{scale}_rep" in filename:
                try:
                    content = json.loads(s3.get_object(Bucket=S3_BUCKET, Key=key)["Body"].read().decode("utf-8"))
...
def analyze_scale(scale_str: str, runs_dict: dict[str, list]):
    ...
    for arch, runs in runs_dict.items():
        ...
        spot_total_cost = (mean_total * spot_rate_sec) + (mean_total * EBS_GP3_PER_SEC_40GB) + (scw_count * 0.000008)
        spot_cost_per_scw = spot_total_cost / scw_count
        
        # On-Demand costs
        ondemand_total_cost = (mean_total * ondemand_rate_sec) + (mean_total * EBS_GP3_PER_SEC_40GB) + (scw_count * 0.000008)
...
def main():
    ...
    print("| Benchmark Scale | Architecture | N | Pipeline Runtime | Total Instance Life | Spot Cost (Run) | Spot Cost / ScW | On-Demand (Run) | On-Demand / ScW |")
    print("| :--- | :--- | :---: | :---: | :---: | :---: | :---: | :---: | :---: |")
    for scale in ["10", "25", "100"]:
        stats = analyze_scale(scale, runs_by_scale[scale])
        for arch in ["x86_64", "arm64"]:
            if arch not in stats:
                continue
            s = stats[arch]
            print(f"| **{scale} ScWs** | `{arch}` | {s['n']} | {s['pipe_seconds']:.1f}s | {s['total_seconds']:.1f}s ({s['total_seconds']/60:.1f}m) | **${s['spot_total_cost']:.4f}** | **${s['spot_cost_per_scw']:.5f}** | ${s['ondemand_total_cost']:.4f} | ${s['ondemand_cost_per_scw']:.5f} |")
    print("\n")
    ...
    x86_100_spot = (1750 * HOURLY_RATES["c7i.xlarge"]["spot"] / 3600) + (1750 * EBS_GP3_PER_SEC_40GB) + (100 * 0.000008)
    x86_100_od = (1750 * HOURLY_RATES["c7i.xlarge"]["ondemand"] / 3600) + (1750 * EBS_GP3_PER_SEC_40GB) + (100 * 0.000008)
    
    arm_100_spot = (2925 * HOURLY_RATES["c7g.xlarge"]["spot"] / 3600) + (2925 * EBS_GP3_PER_SEC_40GB) + (100 * 0.000008)
    arm_100_od = (2925 * HOURLY_RATES["c7g.xlarge"]["ondemand"] / 3600) + (2925 * EBS_GP3_PER_SEC_40GB) + (100 * 0.000008)
