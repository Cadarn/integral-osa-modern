...
def partition(
    ...
    chunk_size: int = typer.Option(
        10, "--chunk-size", "-c", help="Number of ScWs per batch worker"
    ),
    output_dir: Path = typer.Option(
        Path("./k8s/batches"), "--output-dir", "-o", help="Output directory for manifests"
    ),
):
    ...
    console.print(
        f"[bold blue]Partitioning {total_scws} Science Windows into {len(chunks)} worker batches...[/bold blue]"
    )
    ...
    console.print(table)
    console.print(
        f"[bold green]✓ Generated {len(chunks)} batch specifications in {output_dir}[/bold green]"
    )
